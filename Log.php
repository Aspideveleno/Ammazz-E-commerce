...
$redirectUri = 'http://aspid.altervista.org/ProgettoEsame/Log.php';
...
$client2->setClientId($clientID);
$client2->setClientSecret($clientSecret);
$client2->setRedirectUri('http://aspid.altervista.org/ProgettoEsame/Reg.php');
...
if (isset($_GET['code'])) {
  	...
  	if ($result->num_rows > 0)
    {
		...
		//echo "Benvenuto ".$name;//redirect alla pagina principale
		$options = [
			'cost' => 12,
		];
...
else
{
    if ($_SERVER["REQUEST_METHOD"] == "POST") 
    {
        ...
        else if(isset($_POST['UserReg']) && isset($_POST['PassReg']) && isset($_POST['EmailReg']))
        {
        $user = $_POST['UserReg'];
        $pass = $_POST['PassReg'];
        ...
        if($_POST['acceptcookie'] == "Si")
        {
  			...
  			if ($result->num_rows > 0)
    		{
    			$row = $result->fetch_assoc();
				$idd =$row["Id"];
				$sql = "INSERT INTO Carello(id_utente)
        		VALUES ('".$idd."');";
        		$result = $conn->query($sql);
				setcookie("Access", $Hash);
				header("location: P.php");
			}	
        }
    }
        ...
        <span class="underline"></span>
         ...
         <fieldset>
		  ...
		  <div class="input-block">
            ...
            <input id="signup-Email" type="Email" name="EmailReg" required>
          </div>
         Ricordami?
         ...
         <?php
         echo "<a href='".$client2->createAuthUrl()."'>Google Login</a>";
         ?>
         </fieldset>
